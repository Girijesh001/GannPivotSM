...
namespace PivotApplication
{
    ...
    public partial class Window1 : Window
    {
            ...
            public static List<Person> ReadFile(string filepath)
            {
                ...
                var data = from l in lines.Skip(1)
                           ...
                           select new Person
                           {
                               ...
                               EXPIRYDATE = split[3],
                               HIGH = double.Parse(split[7]),
                               LOW = double.Parse(split[8]),
                               CLOSE = double.Parse(split[9]),
                               VOLUME = double.Parse(split[11]),
                           };
        ...
        public void NewMethod(double sellValue, double buyVlaue, string temp1, string temp2, string scriptName,double close)
        {
            ...
            strMessage += "--------------------------------\n";
            Thread obj = new Thread(()=>System.Windows.MessageBox.Show("" + strMessage));
            obj.Start();   
        }
            ...
            {//high close low
                try
                {
                    ...
                    if (diffPivot > Convert.ToDouble(((Person)dgBhavCopy.Items[i]).CLOSE) * .5 / 100 && 
                        ...
                        && ((Person)dgBhavCopy.Items[i]).VOLUME > 1)
                    {
                        ...
                        //    && Convert.ToDouble(((Person)dgBhavCopy.Items[i]).LOW) < limitTo)
                        {
                            var data = new Person
                            {
                                 EXPIRYDATE= ((Person)dgBhavCopy.Items[i]).EXPIRYDATE,
                                ...
                                CLOSE = ((Person)dgBhavCopy.Items[i]).CLOSE,
                                 VOLUME= ((Person)dgBhavCopy.Items[i]).VOLUME
                            };
        ...
        private void button_Click(object sender, RoutedEventArgs e)
        {
            ...
            dlg.DefaultExt = ".csv";
            //dlg.Filter = "CSV Files (*.csv)|*.csv |ZIP Files (*.zip)|*.zip";
